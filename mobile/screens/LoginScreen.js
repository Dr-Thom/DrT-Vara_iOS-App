...
// Maps raw backend / network errors to user-readable messages.
const friendlyError = (err) => {
  ...
  if (raw.includes('cannot reach') || raw.includes('network') || raw.includes('failed to fetch')) {
    return {
      ...
      body: 'We can\'t reach the SAMSON server right now. Please check your internet connection and try again.',
    };
  }
  if (raw.includes('incorrect') || raw.includes('invalid') || raw.includes('credentials') || raw.includes('password') || raw.includes('email')) {
    return {
      title: 'Wrong email or password',
      body: 'Double-check your email and password and try again. Tip: tap "Show" to make sure your password is typed correctly.',
    };
  }
  if (raw.includes('500') || raw.includes('502') || raw.includes('503') || raw.includes('server')) {
    return {
      title: 'SAMSON is having a hiccup',
      body: 'Our server is temporarily unavailable. Please wait a minute and try again.',
    };
  }
  if (raw.includes('429') || raw.includes('too many')) {
    return {
      title: 'Too many attempts',
      body: 'You\'ve tried logging in too many times. Please wait 5 minutes before trying again.',
    };
  }
  // Fallback
  return {
...
const LoginScreen = ({ navigation }) => {
  ...
  const handleLogin = async () => {
    // Trim email (kill autofill whitespace); password kept verbatim but warn on trailing spaces.
    ...
    // Soft warning for whitespace in password (don't auto-trim — passwords can have spaces)
      ...
      Alert.alert(
        ...
        [
          { text: 'Cancel', style: 'cancel' },
          {
            text: 'Trim & log in',
            onPress: async () => {
              setPassword(cleanPassword.trim());
              setTimeout(() => doLogin(cleanEmail, cleanPassword.trim()), 50);
            },
          },
  ...
  const doLogin = async (cleanEmail, cleanPassword) => {
    ...
    try {
      ...
      // Successful — AppNavigator will switch to authed stack
    } catch (error) {
  ...
  const handleForgotPassword = () => {
    Alert.alert(
      ...
      [
        ...
        {
          text: 'Email support',
          onPress: () => Linking.openURL('mailto:varaplatforms@yahoo.com?subject=SAMSON%20Password%20Reset&body=Hi%20Samson%20team%2C%0A%0AI%20need%20to%20reset%20my%20password.%20My%20email%20is%3A%20'),
        },
      ],
    );
  };
  ...
  const fillTestAccount = () => {
    setEmail('admin@vara.com');
    setPassword('vara_admin_2026');
  };
