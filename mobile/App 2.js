...
const AppNavigator = () => {
  ...
  useEffect(() => {
    if (user) hasSeenOnboarding().then(setOnboardingSeen);
    else setOnboardingSeen(null);
  }, [user]);

  if (loading) return null;
  if (user && onboardingSeen === false) return <OnboardingScreen onDone={() => setOnboardingSeen(true)} />;
